...
AITER_BLOCK_SIZE_S3_WORKAROUND="${AITER_BLOCK_SIZE_S3_WORKAROUND:-1}"
...
docker_base_cmd() {
  ...
  -e DISAGG_TRANSFER_BACKEND=${DISAGG_TRANSFER_BACKEND} \
  -e AITER_BLOCK_SIZE_S3_WORKAROUND=${AITER_BLOCK_SIZE_S3_WORKAROUND} \
...
aiter_block_size_s3_workaround_cmd() {
  cat <<'EOF'
  if [ "${AITER_BLOCK_SIZE_S3_WORKAROUND}" = "1" ] && [ "${DISAGG_TRANSFER_BACKEND}" = "mooncake" ]; then
    python3 - <<PY
from pathlib import Path

patch_line = "    config.pop(\"BLOCK_SIZE_S3\", None)\n\n"
patch_comment = "    # Workaround: drop unsupported aiter config key from cached kernels\n"
patch = patch_comment + patch_line

targets = []
try:
    import aiter
    targets.append(Path(aiter.__file__).resolve().parent / "ops/triton/gemm/basic/gemm_afp4wfp4.py")
except Exception as exc:
    print(f"[WARN] Could not import aiter for workaround path resolution: {exc}")

targets.append(Path("/sgl-workspace/aiter/aiter/ops/triton/gemm/basic/gemm_afp4wfp4.py"))
...
for target in unique_targets:
    if not target.exists():
        print(f"[WARN] Workaround target not found: {target}")
        continue
    ...
    if main_new in updated:
        print(f"[INFO] Main gemm_afp4wfp4_ workaround already present in {target}")
        applied = True
    elif main_old in updated:
        updated = updated.replace(main_old, main_new, 1)
        changed = True
        applied = True
    else:
        print(f"[WARN] Main gemm_afp4wfp4_ pattern not found in {target}")
    ...
    if changed:
        ...
        print(f"[INFO] Applied BLOCK_SIZE_S3 workaround to {target}")

if not applied:
    raise SystemExit("[ERROR] BLOCK_SIZE_S3 workaround was requested but not applied")
PY
  fi
EOF
}
...
do_start() {
  ...
  echo "  Backend: ${DISAGG_TRANSFER_BACKEND}"
  echo "=========================================="
...
do_submit() {
  ...
  if ! ssh "${PREFILL_NODE}" "docker exec sglang_r1_prefill python3 -m sglang.bench_serving \
    --backend sglang \
    ...
    --port ${ROUTER_PORT}" 2>&1; then
    echo "[ERROR] Benchmark failed before completion."
    exit 1
  fi
